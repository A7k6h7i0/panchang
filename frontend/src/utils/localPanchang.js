...
function shiftMinutesByOffset(totalMinutes, sourceOffset, targetOffset) {
  const source = parseOffsetToMinutes(sourceOffset);
  const target = parseOffsetToMinutes(targetOffset);
  const shifted = Number(totalMinutes) + (target - source);
  return normalizeMinutes(shifted);
}
...
function clockTextFromMinutes(totalMinutes) {
  const normalized = normalizeMinutes(totalMinutes).minutes;
...
function convertClockTime(timeStr, sourceOffset, targetOffset) {
  const mins = parseClockTimeToMinutes(timeStr);
  if (mins == null) return null;
  const shifted = shiftMinutesByOffset(mins, sourceOffset, targetOffset);
  return clockTextFromMinutes(shifted.minutes);
}

function isoFromSingleTime(dateYmd, timeStr, sourceOffset, targetOffset) {
  const mins = parseClockTimeToMinutes(timeStr);
  if (mins == null) return null;
  const shifted = shiftMinutesByOffset(mins, sourceOffset, targetOffset);
  return localIsoFromMinutes(dateYmd, shifted.minutes, targetOffset, shifted.dayShift);
}

function isoRangeFromTimes(dateYmd, startTime, endTime, sourceOffset, targetOffset) {
  const startMin = parseClockTimeToMinutes(startTime);
  const endMin = parseClockTimeToMinutes(endTime);
  if (startMin == null || endMin == null) {
    return { start: null, end: null };
  }
  const shiftedStart = shiftMinutesByOffset(startMin, sourceOffset, targetOffset);
  const shiftedEnd = shiftMinutesByOffset(endMin, sourceOffset, targetOffset);
  const startAbsolute = shiftedStart.dayShift * 1440 + shiftedStart.minutes;
  let endAbsolute = shiftedEnd.dayShift * 1440 + shiftedEnd.minutes;
  if (endAbsolute <= startAbsolute) {
    endAbsolute += 1440;
  }
  const normalizedEnd = normalizeMinutes(endAbsolute);
  return {
    start: localIsoFromMinutes(dateYmd, shiftedStart.minutes, targetOffset, shiftedStart.dayShift),
    end: localIsoFromMinutes(dateYmd, normalizedEnd.minutes, targetOffset, normalizedEnd.dayShift),
  };
}
...
function toNameAndRange(value, dateYmd, sourceOffset, targetOffset) {
  if (value && typeof value === "object" && !Array.isArray(value)) {
    const name = String(value.name || "").trim();
    const startRaw = String(value.start || "").trim();
    const endRaw = String(value.end || "").trim();
    const range = isoRangeFromTimes(dateYmd, startRaw, endRaw, sourceOffset, targetOffset);
...
export function normalizeDayRecord(day, { tzOffset = "+05:30", sourceTzOffset = "+05:30" } = {}) {
  if (!day || typeof day !== "object") return null;
  const dateYmd = slashDateToYmd(day.date);
  if (!dateYmd) return { ...day };

  const tithi = toNameAndRange(day.Tithi, dateYmd, sourceTzOffset, tzOffset);
  const nakshatra = toNameAndRange(day.Nakshatra, dateYmd, sourceTzOffset, tzOffset);
  const yoga = toNameAndRange(day.Yoga, dateYmd, sourceTzOffset, tzOffset);
  const karana = toNameAndRange(firstNonEmpty(day.Karana, day.Karanam), dateYmd, sourceTzOffset, tzOffset);
  ...
  return {
    ...
    Sunrise: convertClockTime(day.Sunrise, sourceTzOffset, tzOffset) || day.Sunrise || "-",
    Sunset: convertClockTime(day.Sunset, sourceTzOffset, tzOffset) || day.Sunset || "-",
    Moonrise: convertClockTime(day.Moonrise, sourceTzOffset, tzOffset) || day.Moonrise || "-",
    Moonset: convertClockTime(day.Moonset, sourceTzOffset, tzOffset) || day.Moonset || "-",
    SunriseIso: firstNonEmpty(day.SunriseIso, isoFromSingleTime(dateYmd, day.Sunrise, sourceTzOffset, tzOffset)),
    SunsetIso: firstNonEmpty(day.SunsetIso, isoFromSingleTime(dateYmd, day.Sunset, sourceTzOffset, tzOffset)),
    MoonriseIso: firstNonEmpty(day.MoonriseIso, isoFromSingleTime(dateYmd, day.Moonrise, sourceTzOffset, tzOffset)),
    MoonsetIso: firstNonEmpty(day.MoonsetIso, isoFromSingleTime(dateYmd, day.Moonset, sourceTzOffset, tzOffset)),
  };
}
