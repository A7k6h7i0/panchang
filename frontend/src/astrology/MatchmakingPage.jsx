...
export default function MatchmakingPage() {
  ...
  const runFetch = async ({ signal } = {}) => {
    ...
    try {
      ...
      if (status === 429) {
        // Local rate limit safeguard: 5 requests per 60 seconds.
        const until = Date.now() + 60_000;
        setRateLimitedUntil(until);
        setError({
          ...payload,
          message: "You hit the local rate limit (5 requests / 60 seconds). Please wait 60 seconds.",
  ...
  return (
    <PageShell
      title={t.match_making || "Match Making"}
      transparent
      className="matchmaking-page"
      right={
        <Link
          to="/settings"
    ...
    >
        ...
        <form id="matchmaking-form" onSubmit={onSubmit} className="grid gap-4">
          <div className="grid gap-4 md:grid-cols-4">
            ...
            <Field label="Timezone Offset" hint="+05:30">
              <TextInput transparent value={form.tzOffset} onChange={onChange("tzOffset")} />
            </Field>
            <Field label="Language (la)" hint="en/hi/te…">
              <TextInput transparent value={form.la} onChange={onChange("la")} />
            </Field>
            <label className="flex items-end gap-2 rounded-2xl border border-white/10 px-3 py-2 text-sm text-amber-50" style={{ background: "transparent" }}>
              <input
                type="checkbox"
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <SectionCard title="Groom" subtitle="Birth details" transparent>
              <div className="grid gap-4 md:grid-cols-2">
                <Field label="Date">
                  <CalendarDateInput transparent
                ...
                <Field label="Time">
                  <TextInput transparent type="time" value={form.groom.time} onChange={setPerson("groom", "time")} />
                </Field>
                <Field label="Latitude">
                  <TextInput transparent value={form.groom.lat} onChange={setPerson("groom", "lat")} />
                </Field>
                <Field label="Longitude">
                  <TextInput transparent value={form.groom.lng} onChange={setPerson("groom", "lng")} />
                </Field>
              </div>
            </SectionCard>

            <SectionCard title="Bride" subtitle="Birth details" transparent>
              <div className="grid gap-4 md:grid-cols-2">
                <Field label="Date">
                  <CalendarDateInput transparent
                ...
                <Field label="Time">
                  <TextInput transparent type="time" value={form.bride.time} onChange={setPerson("bride", "time")} />
                </Field>
                <Field label="Latitude">
                  <TextInput transparent value={form.bride.lat} onChange={setPerson("bride", "lat")} />
                </Field>
                <Field label="Longitude">
                  <TextInput transparent value={form.bride.lng} onChange={setPerson("bride", "lng")} />
                </Field>
              </div>
            </SectionCard>
          </div>
      ...
      {error ? (
        <SectionCard title="Error" subtitle="Matchmaking data could not be loaded." transparent>
      ...
      {summary ? (
        <SectionCard title="Result" subtitle="Rendered from local matching data (raw JSON optional)." transparent>
          ...
          <div className="mt-3 grid gap-3 md:grid-cols-2">
              <SectionCard title="Manglik (Groom)" transparent>
              ...
              <SectionCard title="Manglik (Bride)" transparent>
              <div className="text-sm text-amber-50">
                <div className="text-amber-100/70">
          ...
          {summary.gunas?.length ? (
            <div className="mt-6">
              <SectionCard title="Ashta Koota (Details)" subtitle="Tap a koota to view description." transparent>
                <Accordion
                  items={summary.gunas}
