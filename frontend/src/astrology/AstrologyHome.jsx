...
import { buildIsoDatetime, findActiveByTime, isoParts, safeDateFromIso, ymdToday } from "./components/formatters";
...
export default function AstrologyHome() {
  ...
  function Tile({ to, title, subtitle }) {
    return (
      <Link
        to={to}
        className="group rounded-3xl border border-white/10 bg-[#2a0607]/75 p-4 shadow-[0_18px_36px_rgba(0,0,0,0.28)] transition hover:-translate-y-0.5 hover:border-amber-300/30 hover:bg-[#350709]/85"
      >
        <div className="text-base font-black tracking-wide text-amber-100 group-hover:text-amber-50">
          {title}
        </div>
        <div className="mt-1 text-xs leading-relaxed text-amber-50/65">{subtitle}</div>
      </Link>
    );
  }
  ...
  const banner = useMemo(() => {
    ...
    const refDate = safeDateFromIso(buildIsoDatetime({ date: form.date, time: form.time, tzOffset: form.tzOffset }));
  ...
  return (
    <PageShell title="Astrology" transparent className="astrology-home" right={null}>
      <div className="grid gap-6">
        <SectionCard
          title="Hindu Calendar"
          subtitle="Choose a date, time, and location to preview the current Panchang summary."
          transparent
        >
          <div className="grid gap-4 md:grid-cols-3">
            <label className="block">
              ...
              <CalendarDateInput
                value={form.date}
                onChange={(next) => setForm((s) => ({ ...s, date: next }))}
                transparent
              />
            </label>
            <label className="block">
          ...
          <div className="mt-4 rounded-2xl border border-white/10 bg-black/20 p-4 text-amber-50">
            <div className="flex flex-wrap items-baseline justify-between gap-3">
              <div className="text-sm font-black tracking-wide text-amber-100">
                {loading ? "Loading..." : banner ? `${banner.vaara || "Day"} • ${form.date}` : "—"}
              </div>
              {banner ? (
        ...
        <div className="grid gap-3 md:grid-cols-2">
          <Tile
            to="/panchang"
            title="Panchang"
            subtitle="Day view • tithi/nakshatra/yoga/karana"
          />
          <Tile
            to="/kundali"
            title="Kundali"
            subtitle="Planet positions • info • dasha"
          />
          <Tile
            to="/matchmaking"
            title="Match Making"
            subtitle="Groom/Bride • guna milan"
          />
          <Tile
            to="/muhurat"
            title="Muhurat"
            subtitle="Auspicious periods"
          />
        </div>
      </div>
    </PageShell>
