...
export default function PanchangPage() {
  ...
  return (
    <PageShell
      title={t.panchang || "Panchang"}
      transparent
      className="panchang-page"
      right={
        <Link
          to="/settings"
    ...
    >
      ...
      >
        <div className="app-panel w-full min-w-0 overflow-x-hidden rounded-2xl p-4" style={{ background: "transparent" }}>
        <form id="panchang-form" onSubmit={onSubmit} className="grid gap-4 md:grid-cols-3">
          ...
          <Field label="Time" hint="HH:MM">
            <div className="grid grid-cols-2 gap-2">
              <SelectInput transparent value={timeHour} onChange={(e) => setTimePart("hour", e.target.value)}>
                {hourOptions.map((h) => (
                  <option key={h} value={h}>{h}</option>
                ))}
              </SelectInput>
              <SelectInput transparent value={timeMinute} onChange={(e) => setTimePart("minute", e.target.value)}>
                {minuteOptions.map((m) => (
          ...
          <Field label="Latitude">
            <TextInput transparent value={form.lat} onChange={onChange("lat")} placeholder="17.3850" />
          </Field>
          <Field label="Longitude">
            <TextInput transparent value={form.lng} onChange={onChange("lng")} placeholder="78.4867" />
          </Field>
      ...
      {error ? (
        <SectionCard title="Error" subtitle="Panchang data could not be loaded." transparent>
      ...
      {summary ? (
        ...
        >
          <div className="grid gap-3 md:grid-cols-2">
            <div className="app-panel rounded-2xl p-4 text-sm text-amber-50" style={{ background: "transparent" }}>
              <div className="grid grid-cols-2 gap-2">
            ...
            <div className="app-surface-soft rounded-2xl p-4 text-sm text-amber-50" style={{ background: "transparent" }}>
