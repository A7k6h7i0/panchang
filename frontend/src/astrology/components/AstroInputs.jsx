...
export function TextInput(props) {
  ...
  const themedStyle = transparent
    ? {
      background: "transparent",
      borderColor: "rgba(255, 183, 77, 0.4)",
      color: "#fff4de",
    }
    : (isDateInput || isTimeInput)
    ? {
      background:
  ...
  return (
    <input
      ...
      style={{
        ...themedStyle,
        ...(transparent ? { colorScheme: "dark" } : null),
        ...props.style,
      }}
    />
  );
...
export function SectionCard({ title, subtitle, children, right, transparent = false }) {
  return (
    <section
      className={`w-full min-w-0 max-w-full overflow-hidden rounded-2xl p-4 ${transparent ? "" : "backdrop-blur-md"}`}
      style={{
        background: transparent
          ? "transparent"
...
export function JsonBlock({ value, transparent = false }) {
  return (
    <pre
