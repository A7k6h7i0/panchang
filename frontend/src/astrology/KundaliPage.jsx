          ...
          <div className="grid gap-2">
            {(Array.isArray(g?.yoga_list) ? g.yoga_list : []).map((y, yIdx) => (
              <div
                ...
                className="rounded-2xl border border-white/10 p-3 sm:p-4 text-sm text-amber-50"
...
export default function KundaliPage() {
  ...
  return (
    <PageShell
      title={t.kundali || "Kundali"}
      transparent
      className="kundali-page"
      right={
        <Link
          to="/settings"
    ...
    >
        ...
        right={
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              form="kundali-form"
              disabled={loading}
              className="rounded-xl px-4 py-2 text-sm font-black text-amber-100 ring-1 ring-amber-300/30 transition hover:bg-amber-400/25 disabled:cursor-not-allowed disabled:opacity-60"
              style={{ background: "transparent" }}
            >
      ...
      >
        <form id="kundali-form" onSubmit={onSubmit} className="grid gap-4 md:grid-cols-3">
          <Field label="Date" hint="YYYY-MM-DD">
            <CalendarDateInput transparent value={form.date} onChange={(next) => setForm((s) => ({ ...s, date: next }))} />
          </Field>
          <Field label="Time" hint="HH:MM">
            <TextInput transparent type="time" value={form.time} onChange={onChange("time")} />
          </Field>
          <Field label="Location Name" hint="Optional (for display)">
            <TextInput transparent value={form.locationName} onChange={onChange("locationName")} placeholder="Ujjain, India" />
          </Field>
          <Field label="Timezone Offset" hint="+05:30">
            <TextInput transparent value={form.tzOffset} onChange={onChange("tzOffset")} placeholder="+05:30" />
          </Field>

          <Field label="Latitude">
            <TextInput transparent value={form.lat} onChange={onChange("lat")} placeholder="23.1500" />
          </Field>
          <Field label="Longitude">
            <TextInput transparent value={form.lng} onChange={onChange("lng")} placeholder="75.7700" />
          </Field>
          <Field label="Ayanamsa" hint="1, 3, 5">
            <SelectInput transparent value={form.ayanamsa} onChange={onChange("ayanamsa")}>
          ...
          <Field label="Language (la)" hint="en/hi/te…">
            <TextInput transparent value={form.la} onChange={onChange("la")} placeholder="en" />
          </Field>
        </form>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {[
            { id: "info", label: "Info" },
            { id: "kundali", label: t.kundali || "Kundali" },
            ...
            <button
              ...
              className={`rounded-xl px-3 py-2 text-sm font-black ring-1 transition ${
                tab === tabItem.id
                  ? "text-amber-100 ring-amber-300/30"
                  : "text-amber-100/80 ring-white/10 hover:bg-white/10"
              }`}
              style={{ background: "transparent" }}
            >
              {tabItem.label}
      ...
      {error ? (
        <SectionCard title="Error" subtitle="Kundali data could not be loaded." transparent>
      ...
      {tab === "info" && info ? (
        <SectionCard title="Info" transparent>
          <div className="mb-4 rounded-2xl border border-white/10 p-4 text-sm text-amber-50" style={{ background: "transparent" }}>
          ...
          {(info.rashiSwami || info.nakshatraSwami || info.yoni || info.gana || info.nadi) ? (
            <div className="mt-4 rounded-2xl border border-white/10 p-4 text-sm text-amber-50" style={{ background: "transparent" }}>
              <div className="grid grid-cols-2 gap-2">
      ...
      {tab === "kundali" ? (
        <div className="grid gap-6">
          {planetSets.length ? (
            <>
              {lagnaRow?.rashi ? (
            <SectionCard title="Kundali Chart" subtitle="North Indian chart (best-effort from Lagna + planet rashi)." transparent>
              <NorthIndianChart lagnaRashi={lagnaRow.rashi} planets={nirayanaRows} />
            </SectionCard>
          ) : (
            <SectionCard title="Kundali Chart" subtitle="Chart needs Lagna rashi from the response." transparent>
              <div className="rounded-2xl border border-white/10 p-4 text-sm text-amber-50" style={{ background: "transparent" }}>
                Lagna not found yet. Click “Generate” and try again if needed.
              </div>
            </SectionCard>
          )}
          {planetSets.map((set) => (
            <SectionCard
              ...
              transparent
            >
              <div className="rounded-2xl border border-white/10 overflow-hidden" style={{ background: "transparent" }}>
                <table className="hidden w-full text-left text-sm text-amber-50 md:table">
                  <thead className="text-amber-100/80" style={{ background: "transparent" }}>
                    <tr>
                  ...
                  <tbody className="divide-y divide-white/10" style={{ background: "transparent" }}>
                    {set.rows.map((p, idx) => {
                ...
                <div className="grid gap-2 p-2 md:hidden">
                  {set.rows.map((p, idx) => {
                    const row = normalizePlanetRow(p);
                    return (
                      <div key={`${row.name}-${idx}`} className="rounded-xl border border-white/10 p-2 text-xs text-amber-50" style={{ background: "transparent" }}>
            ...
            <SectionCard title="Kundali" transparent>
              <div className="rounded-2xl border border-white/10 p-4 text-sm text-amber-50" style={{ background: "transparent" }}>
          ...
          <SectionCard
            ...
            right={
              <button
                ...
                className="rounded-xl px-3 py-2 text-sm font-black text-amber-100/80 ring-1 ring-white/10 transition hover:bg-white/10"
                style={{ background: "transparent" }}
              >
                {showYogas ? "Hide" : "Show"}
              </button>
      ...
      {tab === "dasha" && root ? (
        <SectionCard title="Dasha" transparent>
