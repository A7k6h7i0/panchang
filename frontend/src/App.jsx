...
import { ensureLanguageLoaded, initialLanguageReady, translations, languages } from "./translations";
...
const fetchFestivalMap = async (year) => {
  try {
    ...
    setCachedFestivalData(year, data);
    return data;
  } catch {
    return {};
  }
...
function App() {
  ...
  const monthHeaderTouchRef = useRef({ x: 0, y: 0, active: false });
  const monthDayCarouselRef = useRef(null);
  const monthDayCarouselScrollTimeoutRef = useRef(null);
  const monthDayCarouselSyncingRef = useRef(false);
  const [monthDayCarouselRecords, setMonthDayCarouselRecords] = useState([]);
  const hasAutoScrolledRef = useRef(false);
  const festivalTranslationsRef = useRef({});
  const prevCountryRef = useRef(loadCountry());
  ...
  useEffect(() => {
    ...
    const nextYear = countryNowParts.year;
    const nextMonth = countryNowParts.month - 1;
    const nextDay = countryNowParts.day;

    setYear(nextYear);
    setMonth(nextMonth);
    setPreferredDay(nextDay);
    setTempYear(nextYear);
    setTempMonth(nextMonth);
    setTempDay(nextDay);
    setShowDatePicker(false);
  }, [country, countryNowParts.year, countryNowParts.month, countryNowParts.day]);
  ...
  useEffect(() => {
    ...
    const syncLanguage = (event) => {
      ...
      ensureLanguageLoaded(saved).finally(() => {
        setLanguage((prev) => (prev === saved ? prev : saved));
      });
    };
  ...
  useEffect(() => {
    ...
    const applyCalendarData = (yearData, festivalMap) => {
      if (cancelled) return;
    ...
    const cachedYear = getCachedYearData(year);
    const cachedFestival = getCachedFestivalData(year);
    const hasCachedPair = Array.isArray(cachedYear?.data) && cachedFestival?.data;

    if (hasCachedPair) {
      applyCalendarData(cachedYear.data, cachedFestival.data);
    }

    const yearFresh = isCacheFresh(cachedYear);
    const festivalFresh = isCacheFresh(cachedFestival);
    if (hasCachedPair && yearFresh && festivalFresh) {
      return () => {
        cancelled = true;
      };
    }
    ...
    Promise.all([
      ...
      .then(([yearData, festivalMap]) => {
        applyCalendarData(yearData, festivalMap);
      })
      .catch((error) => {
        if (cancelled) return;
        if (hasCachedPair) return;
  ...
  const goPrevMonth = () => {
    ...
    Promise.all([
      getCachedYearData(newYear)?.data ? Promise.resolve(getCachedYearData(newYear).data) : fetchYearData(newYear),
      getCachedFestivalData(newYear)?.data ? Promise.resolve(getCachedFestivalData(newYear).data) : fetchFestivalMap(newYear),
    ])
  ...
  const handleMonthHeaderTouchStart = (event) => {
    const touch = event.touches?.[0];
    if (!touch) return;
    monthHeaderTouchRef.current = {
      x: touch.clientX,
      y: touch.clientY,
      active: true,
    };
  ...
  const handleMonthHeaderTouchEnd = (event) => {
    const start = monthHeaderTouchRef.current;
    const touch = event.changedTouches?.[0];
    monthHeaderTouchRef.current = { x: 0, y: 0, active: false };
    if (!start.active || !touch) return;

    const deltaX = touch.clientX - start.x;
    const deltaY = touch.clientY - start.y;
    if (Math.abs(deltaX) < 40 || Math.abs(deltaY) > 30) return;
  ...
  const monthDayCarouselDateKeys = useMemo(() => {
    const centerYmd = slashDateToYmd(selectedDay?.date || formatDateString(year, month, preferredDay));
    if (!centerYmd) return [];
    return [shiftDateKey(centerYmd, -1), centerYmd, shiftDateKey(centerYmd, 1)];
  }, [selectedDay?.date, year, month, preferredDay]);

  useEffect(() => {
    let active = true;

    const loadSlides = async () => {
      if (!monthDayCarouselDateKeys.length) {
        if (active) setMonthDayCarouselRecords([]);
        return;
      }

      const records = await Promise.all(
        monthDayCarouselDateKeys.map(async (ymd) => {
          if (!ymd) return null;
          const rawRecord = await findLocalDayByYmd(ymd).catch(() => null);
          if (!rawRecord) return null;
          const yearNum = Number(String(ymd).slice(0, 4));
          const festivalMap = getCachedFestivalData(yearNum)?.data || (await fetchFestivalMap(yearNum));
          const merged = withFestivalsFromMap(rawRecord, festivalMap);
          return normalizeDayRecord(merged, {
            tzOffset: countryDefaults.tzOffset,
            sourceTzOffset: LOCAL_DATA_TZ_OFFSET,
          });
        })
      );
      ...
      if (!active) return;
      setMonthDayCarouselRecords(records.filter(Boolean));
    };
  ...
  }, [monthDayCarouselDateKeys, countryDefaults.tzOffset]);

  const selectAdjacentDay = useCallback((delta) => {
    const nextDay = delta < 0 ? monthDayCarouselRecords[0] : monthDayCarouselRecords[2];
    if (!nextDay) return;
    setSelectedDay(nextDay);
    const [dayStr, monthStr, yearStr] = String(nextDay.date || "").split("/");
    ...
    if (Number.isFinite(dayNum) && Number.isFinite(monthNum) && Number.isFinite(yearNum)) {
      ...
      setTempYear(yearNum);
      setTempMonth(monthNum - 1);
      setTempDay(dayNum);
    }
  }, [monthDayCarouselRecords]);

  const recenterMonthDayCarousel = useCallback((behavior = "auto") => {
    const el = monthDayCarouselRef.current;
    if (!el) return;
    monthDayCarouselSyncingRef.current = true;
    el.scrollTo({ left: el.clientWidth, behavior });
    window.clearTimeout(monthDayCarouselScrollTimeoutRef.current);
    monthDayCarouselScrollTimeoutRef.current = window.setTimeout(() => {
      monthDayCarouselSyncingRef.current = false;
    }, behavior === "smooth" ? 220 : 60);
  }, []);

  useEffect(() => {
    if (!selectedDay?.date) return;
    if (!monthDayCarouselRecords.length) return;
    const raf = window.requestAnimationFrame(() => {
      recenterMonthDayCarousel("auto");
    });
    return () => window.cancelAnimationFrame(raf);
  }, [selectedDay?.date, monthDayCarouselRecords.length, recenterMonthDayCarousel]);

  const handleMonthDayCarouselScroll = useCallback(() => {
    if (monthDayCarouselSyncingRef.current) return;
    const el = monthDayCarouselRef.current;
    if (!el) return;
    window.clearTimeout(monthDayCarouselScrollTimeoutRef.current);
    monthDayCarouselScrollTimeoutRef.current = window.setTimeout(() => {
  ...
  }, [recenterMonthDayCarousel, selectAdjacentDay]);
  ...
  const selectedDayWithLocalData = useMemo(
    ...
    [countryDefaults.tzOffset, selectedDay]
  );
  ...
  const handleDatePickerOk = (data) => {
    ...
    Promise.all([
      ...
      dayData
        ...
        : (getCachedYearData(y)?.data ? Promise.resolve(getCachedYearData(y).data) : fetchYearData(y)),
    ])
    ...
    >
      ...
      <header className="relative z-40">
        ...
        <div className="relative mx-auto max-w-7xl px-3 sm:px-6 lg:px-8 pt-2 sm:pt-3 pb-0.5 sm:pb-1">
          ...
          <div className="mb-0.5 flex items-center justify-between border-b-[1.5px] border-[rgba(255,183,77,0.4)] pb-0.5">
          ...
          <div className="relative mt-px">
            <div
              ref={monthDayCarouselRef}
              onScroll={handleMonthDayCarouselScroll}
              className="hide-scrollbar flex w-full snap-x snap-mandatory overflow-x-auto overflow-y-hidden"
              style={{
                WebkitOverflowScrolling: "touch",
              }}
            >
              {monthDayCarouselRecords.map((daySlide, idx) => (
                <div key={`${daySlide?.date || "day"}-${idx}`} className="w-full shrink-0 snap-center">
                  <DayDetails
                    day={daySlide}
                    language={language}
                    translations={t}
                  isHeaderMode={true}
                  clockTimeZone={countryDefaults.timeZone}
                  onDateClick={openDatePicker}
                  festivals={daySlide?.Festivals || null}
                  voiceEnabled={voiceEnabled}
                  compactTithiRange={true}
                  onPrevDay={() => selectAdjacentDay(-1)}
                  onNextDay={() => selectAdjacentDay(1)}
                />
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1.5fr_1fr] gap-1 sm:gap-1">
            ...
            <section
              ...
              <div
                ...
                style={{
                  borderBottom: "1.5px solid rgba(255, 183, 77, 0.4)",
                  background: "transparent",
                  borderRadius: "12px",
                  touchAction: "pan-y",
                }}
                onTouchStart={handleMonthHeaderTouchStart}
                onTouchEnd={handleMonthHeaderTouchEnd}
              >
              ...
              >
                  <span onClick={(e) => { e.stopPropagation(); goPrevMonth(); }} style={{ cursor: 'pointer', color: 'var(--tc-base)' }}>←</span>
                  <span style={{ color: "var(--tc-base)" }}>{monthLabel}</span>
                  <span onClick={(e) => { e.stopPropagation(); goNextMonth(); }} style={{ cursor: 'pointer', color: 'var(--tc-base)' }}>→</span>
                </button>
              ...
              >
                  <span style={{ color: "var(--tc-label)" }}>📅</span>
                  <span>{year}</span>
                </button>
              </div>
            ...
            >
              ...
              <DayDetails
                ...
                isSidebarMode={true}
                clockTimeZone={countryDefaults.timeZone}
                festivals={selectedDayWithLocalData?.Festivals || null}
                voiceEnabled={voiceEnabled}
