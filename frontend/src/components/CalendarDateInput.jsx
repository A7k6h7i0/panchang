...
export default function CalendarDateInput({
  ...
  return (
    <>
      <div
        className={`flex w-full min-w-0 max-w-full items-center gap-2 box-border rounded-xl border px-3 py-2 text-amber-50 outline-none transition ${className}`}
        style={{
          background: transparent ? "transparent" : "linear-gradient(135deg, #2a5a1f 0%, #3a6e2d 30%, #4a8238 60%, #5a9645 100%)",
      ...
      >
        <input
          ...
          className="min-w-0 flex-1 bg-transparent text-left font-semibold text-amber-50 outline-none placeholder:text-amber-100/60"
        />
        <button
          type="button"
          onMouseDown={(e) => e.stopPropagation()}
          onClick={() => setIsOpen(true)}
          className="shrink-0 rounded-lg px-2 py-1 text-base leading-none text-amber-100 transition hover:bg-white/10"
          aria-label={`Open date picker${value ? ` for ${displayValue}` : ""}`}
        >
          {"\uD83D\uDCC5"}
        </button>
      </div>
