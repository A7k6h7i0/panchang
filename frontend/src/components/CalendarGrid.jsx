...
export default function CalendarGrid({
  ...
  const fallbackTodayStr = `${String(today.getDate()).padStart(2, "0")}/${String(
    today.getMonth() + 1
  ).padStart(2, "0")}/${today.getFullYear()}`;
  const todayStr = todayDate || fallbackTodayStr;

  const paddedDays = [...Array(firstDay).fill(null), ...days];

  return (
    <div
      className="relative rounded-2xl p-2 sm:p-4 neon-frame overflow-hidden"
      style={{
        background: "linear-gradient(180deg, rgba(28, 78, 42, 0.98) 0%, rgba(18, 58, 30, 0.98) 100%)",
    ...
    >
      ...
      <div className="relative">
        ...
        <div className="mb-3 grid grid-cols-7 gap-0.5 sm:gap-2">
          {translations.weekdaysShort.map((wd, idx) => {
            const weekdayText = String(wd || "");

            return (
            <div
              key={idx}
              className="flex min-h-[2.5rem] min-w-0 items-center justify-center overflow-hidden px-0.5 py-1 text-[7px] sm:text-xs font-normal tracking-normal uppercase text-center leading-none whitespace-nowrap rounded-xl"
              style={{
                color: "#c8ddc1",
              ...
              title={weekdayText}
            >
              <span className="block w-full min-w-0 text-center">{weekdayText}</span>
            </div>
            );
          })}
        </div>
            ...
            return (
              <button
                ...
                style={
                  isSelected && isToday
                    ? {
                        background: "linear-gradient(180deg, rgba(255, 214, 102, 0.38) 0%, rgba(255, 183, 77, 0.24) 100%)",
                        borderColor: "rgba(255, 214, 102, 1)",
                        boxShadow: "0 0 0 2px rgba(255, 214, 102, 0.3), inset 0 0 26px rgba(255, 214, 102, 0.18)",
                      }
                    : isSelected
                      ? {
                          background: "linear-gradient(180deg, rgba(255, 214, 102, 0.26) 0%, rgba(255, 183, 77, 0.14) 100%)",
                          borderColor: "rgba(255, 214, 102, 0.95)",
                          boxShadow: "0 0 0 2px rgba(255, 214, 102, 0.2), inset 0 0 22px rgba(255, 214, 102, 0.1)",
                      }
                      : isToday
                        ? {
                            background: "linear-gradient(180deg, rgba(255, 214, 102, 0.34) 0%, rgba(255, 183, 77, 0.2) 100%)",
                            borderColor: "rgba(255, 214, 102, 0.95)",
                            boxShadow: "0 0 0 2px rgba(255, 214, 102, 0.28), inset 0 0 22px rgba(255, 214, 102, 0.16)",
                          }
                        : {
                            background: "transparent",
                ...
                onMouseEnter={(e) => {
                  if (!(isSelected || isToday)) {
                    e.currentTarget.style.borderColor = "rgba(255, 183, 77, 0.4)";
                    e.currentTarget.style.boxShadow = "none";
                  }
                }}
                onMouseLeave={(e) => {
                  if (isSelected && isToday) {
                    e.currentTarget.style.borderColor = "rgba(255, 214, 102, 1)";
                    e.currentTarget.style.boxShadow = "0 0 0 2px rgba(255, 214, 102, 0.3), inset 0 0 26px rgba(255, 214, 102, 0.18)";
                  } else if (isSelected) {
                    e.currentTarget.style.borderColor = "rgba(255, 214, 102, 0.95)";
                    e.currentTarget.style.boxShadow = "0 0 0 2px rgba(255, 214, 102, 0.2), inset 0 0 22px rgba(255, 214, 102, 0.1)";
                  } else if (isToday) {
                    e.currentTarget.style.borderColor = "rgba(255, 214, 102, 0.95)";
                    e.currentTarget.style.boxShadow = "0 0 0 2px rgba(255, 214, 102, 0.28), inset 0 0 22px rgba(255, 214, 102, 0.16)";
                  } else {
                    e.currentTarget.style.borderColor = "rgba(255, 183, 77, 0.4)";
                    e.currentTarget.style.boxShadow = "none";
                  }
