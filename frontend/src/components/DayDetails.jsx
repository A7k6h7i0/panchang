...
import { findLocalDayByYmd, loadYearData, normalizeDayRecord } from "../utils/localPanchang";
import { formatTimeInTimeZone, getTimePartsInTimeZone } from "../utils/appSettings";
...
import {
  ...
  getMuhurtaName,
  normalizeSpeechTimeRange,
  isAuspiciousMuhurta,
} from "../utils/speechTemplates";
import { preloadTTS, speakCloud, stopSpeech } from "../utils/cloudSpeech";
import { postFlutterMessage } from "../utils/flutterBridge";


const API_BASE_URL = import.meta.env.VITE_API_BASE_URL || "http://localhost:5000";
const MONTH_VIEW_SHARED_BORDER = "1.5px solid rgba(255, 183, 77, 0.4)";
const MONTH_VIEW_HEADING_COLOR = "#FFE4B5";
const MONTH_VIEW_CONTENT_COLOR = "#08C849";
const MONTH_VIEW_TIME_COLOR = "#8FD7FF";
const MONTH_VIEW_SECTION_TITLE_COLOR = "#FFFFFF";
const MONTH_VIEW_SECTION_TITLE_SHADOW = "0 0 10px rgba(255, 255, 255, 0.65), 0 1px 2px rgba(0, 0, 0, 0.45)";
...
const slashDateToYmd = (dateStr) => {
  const [day, month, year] = String(dateStr || "").split("/");
  if (!day || !month || !year) return "";
  return `${year}-${month}-${day}`;
};

const formatPanchangStartLine = (entry, monthNames, monthNamesShort, timeZone, startsLabel, displayDateKey) => {
  if (!entry || typeof entry !== "object") return "";
  const start = entry.start ? new Date(entry.start) : null;
  if (!start || Number.isNaN(start.getTime())) return "";
  const displayDate = displayDateKey ? new Date(`${displayDateKey}T12:00:00`) : null;
  ...
  return `(${entry?.name || ""} ${startsLabel || "starts"} ${timeText} on ${day} ${monthName})`;
};

const buildNextDetailRecord = (dayRecord, fieldName, fallbackStartField) => {
...
function buildChipMeta(value, language, translations, timeText = "") {
  ...
  const fallbackTime = hasRange ? `${startTime} - ${endTime}` : hasSingle ? startTime : "";
...
const resolveSpeechLanguage = (language) => {
...
function buildMuhurtaTimingSpeech({ language, key, value }) {
  const timeText = String(value || "").trim();
  if (!timeText || timeText === "-") return "";

  const resolvedLanguage = resolveSpeechLanguage(language);
  const name = getMuhurtaName(key, resolvedLanguage);
  const spokenTime = normalizeSpeechTimeRange({
    language: resolvedLanguage,
    timeText,
  });
  return `${name} ${spokenTime}.`;
}

function SpeakTimingButton({ label, speechKey, value, language }) {
  if (!speechKey || !value || value === "-") return null;

  const handleSpeak = async () => {
    const text = buildMuhurtaTimingSpeech({ language, key: speechKey, value });
    if (!text) return;
    stopSpeech();
  ...
  return (
    <button
      ...
      style={{
        ...
        border: "1.5px solid rgba(255, 237, 179, 0.55)",
        color: "#ffedb3",
      }}
    >
      <svg width="12" height="12" viewBox="0 0 24 24" aria-hidden="true" focusable="false">
        <path
          fill="currentColor"
          d="M12 15a3 3 0 0 0 3-3V6a3 3 0 1 0-6 0v6a3 3 0 0 0 3 3zm5-3a5 5 0 0 1-10 0H5a7 7 0 0 0 6 6.92V21h2v-2.08A7 7 0 0 0 19 12z"
        />
      </svg>
    </button>
  );
...
function MuhurthaTimer({ startTime, endTime, isAuspicious, language, translations }) {
  ...
  // Keep the timer color tied strictly to the muhurta type.
...
export default function DayDetails({
  ...
  const [nextDayRecordNormalized, setNextDayRecordNormalized] = useState(null);
  ...
  useEffect(() => {
    ...
    const directFestivals = Array.isArray(festivalsProp) && festivalsProp.length > 0
      ? festivalsProp
      : Array.isArray(day?.Festivals)
        ? day.Festivals
        : [];
  ...
  useEffect(() => {
    ...
    const dateStr = day?.date;
    if (!dateStr) {
      setNextDayRecordNormalized(null);
      return;
    }

    const dateKey = slashDateToYmd(dateStr);
    if (!dateKey) {
      setNextDayRecordNormalized(null);
      return;
    }
    ...
    findLocalDayByYmd(nextDateKey)
      .then((record) => {
        if (!active) return;
        setNextDayRecordNormalized(
          record
            ? normalizeDayRecord(record, {
                tzOffset: "+05:30",
                sourceTzOffset: "+05:30",
              })
            : null
        );
      })
      .catch(() => {
        if (active) setNextDayRecordNormalized(null);
      });

    return () => {
      active = false;
    };
  }, [day?.date, clockTimeZone]);
  ...
  const formatStartEndDate = (startIso, endIso, includeDates = true) => {
    ...
    const startDate = new Intl.DateTimeFormat("en-US", dateOptions).format(start);
    const endTime = new Intl.DateTimeFormat("en-US", timeOptions).format(end);
    const endDate = new Intl.DateTimeFormat("en-US", dateOptions).format(end);
    if (!includeDates) {
  ...
  const referenceDate = day?.date || "";
  const localizedMonths = translations.months || [];
  const localizedMonthsShort = translations.monthsShort || [];
  const displayDateKey = slashDateToYmd(day?.date || "");
  const renderRange = (startIso, endIso) =>
    startIso || endIso ? (
      <LocalizedTimeRange
        startIso={startIso}
        endIso={endIso}
        ...
        monthNamesShort={localizedMonthsShort}
      />
    ) : null;
  const tithiRange = renderRange(day?.TithiStart, day?.TithiEnd);
  const nakshatraRange = renderRange(day?.NakshatraStart, day?.NakshatraEnd);
  const yogaRange = renderRange(day?.YogaStart, day?.YogaEnd);
  const karanaRange = renderRange(day?.KaranaStart, day?.KaranaEnd);
  const pakshaRange = tithiRange;
  const tithiDetail = formatPanchangStartLine(
    buildNextDetailRecord(nextDayRecordNormalized, "Tithi"),
    localizedMonths,
  ...
  const nakshatraDetail = formatPanchangStartLine(
    buildNextDetailRecord(nextDayRecordNormalized, "Nakshatra"),
    localizedMonths,
    localizedMonthsShort,
    clockTimeZone || "Asia/Kolkata",
    translations.starts,
  ...
  const yogaDetail = formatPanchangStartLine(
    buildNextDetailRecord(nextDayRecordNormalized, "Yoga"),
    localizedMonths,
    localizedMonthsShort,
    clockTimeZone || "Asia/Kolkata",
    translations.starts,
  ...
  const pakshaDetail = formatPanchangStartLine(
    buildNextDetailRecord(nextDayRecordNormalized, "Paksha", "TithiStart"),
    localizedMonths,
    localizedMonthsShort,
    clockTimeZone || "Asia/Kolkata",
    translations.starts,
  ...
  const karanaDetail = formatPanchangStartLine(
    buildNextDetailRecord(nextDayRecordNormalized, "Karana"),
    localizedMonths,
    localizedMonthsShort,
    clockTimeZone || "Asia/Kolkata",
    translations.starts,
  ...
  useEffect(() => {
    ...
    const entries = [
      { key: "Amrit Kalam", value: vAmrit },
      { key: "Abhijit", value: vAbhijit },
      { key: "Rahu Kalam", value: vRahu },
      { key: "Yamaganda", value: vYamaganda },
      { key: "Gulikai Kalam", value: vGulikai },
      { key: "Dur Muhurtam", value: vDur },
      { key: "Varjyam", value: vVarjyam },
    ]
      .filter((entry) => entry.value && entry.value !== "-")
      .map((entry) => {
        const text = buildMuhurtaTimingSpeech({
          language,
          key: entry.key,
          value: entry.value,
        });
  ...
  const headerTime = clockTimeZone ? formatIsoDateTime(headerNow.toISOString()) : headerNow.toLocaleTimeString("en-US", {
    hour: "numeric",
    minute: "2-digit",
    hour12: true,
  });
  const displayedHeaderTime = clockTimeZone ? formatTimeInTimeZone(headerNow, clockTimeZone) : headerTime;
  const headerHinduTime = (() => {
    ...
    const nowParts = clockTimeZone
      ? getTimePartsInTimeZone(headerNow, clockTimeZone)
      : { hour: headerNow.getHours(), minute: headerNow.getMinutes() };
  ...
  if (isHeaderMode) {
    const tithiMeta = buildChipMeta(vTithi, language, translations, tithiRange);
    const nakshatraMeta = buildChipMeta(vNakshatra, language, translations, nakshatraRange);
    const yogaMeta = buildChipMeta(vYoga, language, translations, yogaRange);
    const pakshaMeta = buildChipMeta(vPaksha, language, translations, pakshaRange);
    const karanaMeta = buildChipMeta(vKarana, language, translations, karanaRange);
      ...
      return (
        ...
        >
          <div className="flex min-w-0 flex-col leading-tight text-center">
            <span className="month-view-label-text truncate" style={{ color: MONTH_VIEW_HEADING_COLOR }}>
              {icon} {meta.label}
            </span>
            {meta.time ? (
              <span className="month-view-time-text text-[10px] leading-tight whitespace-normal" style={{ color: MONTH_VIEW_TIME_COLOR }}>
                {meta.time}
              </span>
            ) : null}
          </div>
    ...
    return (
      <>
        <div
          ...
          style={{
            background: "transparent",
            border: MONTH_VIEW_SHARED_BORDER,
            boxShadow: "none",
          }}
        >
          ...
          <button
            type="button"
            className="h-12 w-12 sm:h-16 sm:w-16 rounded-lg flex items-center justify-center flex-shrink-0"
            onClick={onDateClick || undefined}
            style={{
              background: "transparent",
              border: MONTH_VIEW_SHARED_BORDER,
              padding: 0,
            ...
            aria-label={onDateClick ? "Open date picker" : undefined}
          >
            <span
            ...
            <div className="flex-1 min-w-0 text-left">
              ...
              <div
                    ...
                    style={{
                      color: MONTH_VIEW_HEADING_COLOR,
                ...
                {headerHinduTime && headerHinduTime !== "-" && (
                  ...
                  >
                    <span className="month-view-label-text" style={{ color: MONTH_VIEW_HEADING_COLOR }}>Hindu Time:</span>
                    <span className="month-view-time-text ml-1" style={{ color: MONTH_VIEW_TIME_COLOR }}>{headerHinduTime}</span>
                  </div>
                )}
              </div>
              <div className="text-xs sm:text-sm font-medium">
                <button
                  type="button"
                  onClick={onDateClick || undefined}
                  className="month-view-header-date month-view-value-text"
                  style={{
                    color: "#FFFFFF",
                ...
                <span className="month-view-time-text whitespace-nowrap" style={{ color: MONTH_VIEW_TIME_COLOR }}>, {displayedHeaderTime}</span>
              </div>
            </div>
          </div>
          ...
          {onPrevDay || onNextDay ? (
            <div className="mt-1 flex items-center justify-between gap-2 sm:gap-3">
              ...
              >
                <UiIcon name="arrowLeft" size={12} color={MONTH_VIEW_HEADING_COLOR} />
              </button>
              <button
                type="button"
          ...
          {vTithi && vTithi !== "-" && (
            <div
              className="mt-1 mb-1 flex min-w-0 flex-col gap-0.5 rounded-full pl-0.5 pr-2.5 py-1 text-[11px] sm:text-xs font-bold"
              style={{
              background: "transparent",
              border: "0",
              color: MONTH_VIEW_HEADING_COLOR,
            ...
            >
              <span className="min-w-0 truncate whitespace-nowrap">
                <span className="month-view-label-text" style={{ color: MONTH_VIEW_HEADING_COLOR }}>{translations.tithi || "Tithi"}:</span>{" "}
                <span className="month-view-value-text" style={{ color: MONTH_VIEW_CONTENT_COLOR }}>{vTithi}</span>
              </span>
              {tithiRange}
              {tithiDetail ? (
                <span className="month-view-time-text month-view-detail-bracket mt-0.5 block text-[10px] leading-tight whitespace-normal" style={{ color: "#FFFFFF" }}>
                  {tithiDetail}
                </span>
              ) : null}
            </div>
          ...
          {vNakshatra && vNakshatra !== "-" && (
            ...
            >
              <span className="min-w-0 truncate whitespace-nowrap">
                <span className="month-view-label-text" style={{ color: MONTH_VIEW_HEADING_COLOR }}>{translations.nakshatra || "Nakshatra"}:</span>{" "}
                <span className="month-view-value-text" style={{ color: MONTH_VIEW_CONTENT_COLOR }}>{vNakshatra}</span>
              </span>
              {nakshatraRange}
              {nakshatraDetail ? (
                <span className="month-view-time-text month-view-detail-bracket mt-0.5 block text-[10px] leading-tight whitespace-normal" style={{ color: "#FFFFFF" }}>
          ...
          {vYoga && vYoga !== "-" && (
            ...
            >
              <span className="min-w-0 truncate whitespace-nowrap">
                <span className="month-view-label-text" style={{ color: MONTH_VIEW_HEADING_COLOR }}>{translations.yoga || "Yoga"}:</span>{" "}
                <span className="month-view-value-text" style={{ color: MONTH_VIEW_CONTENT_COLOR }}>{vYoga}</span>
              </span>
              {yogaRange}
              {yogaDetail ? (
                <span className="month-view-time-text month-view-detail-bracket mt-0.5 block text-[10px] leading-tight whitespace-normal" style={{ color: "#FFFFFF" }}>
          ...
          {vPaksha && vPaksha !== "-" && (
            ...
            >
              <span className="min-w-0 truncate whitespace-nowrap">
                <span className="month-view-label-text" style={{ color: MONTH_VIEW_HEADING_COLOR }}>{translations.paksha || "Paksha"}:</span>{" "}
                <span className="month-view-value-text" style={{ color: MONTH_VIEW_CONTENT_COLOR }}>{vPaksha}</span>
              </span>
              {pakshaRange}
              {pakshaDetail ? (
                <span className="month-view-time-text month-view-detail-bracket mt-0.5 block text-[10px] leading-tight whitespace-normal" style={{ color: "#FFFFFF" }}>
          ...
          {day?.Karana && day.Karana !== "-" && (
            ...
            >
              <span className="min-w-0 truncate whitespace-nowrap">
                <span className="month-view-label-text" style={{ color: MONTH_VIEW_HEADING_COLOR }}>{translations.karana || "Karana"}:</span>{" "}
                <span className="month-view-value-text" style={{ color: MONTH_VIEW_CONTENT_COLOR }}>{vKarana}</span>
              </span>
              {karanaRange}
              {karanaDetail ? (
                <span className="month-view-time-text month-view-detail-bracket mt-0.5 block text-[10px] leading-tight whitespace-normal" style={{ color: "#FFFFFF" }}>
          ...
          {day?.Choghadiya && day.Choghadiya !== "-" && (
            <div
              className="mt-1 mb-1 flex min-w-0 items-center justify-between gap-1 rounded-full pl-0.5 pr-2.5 py-1 text-[11px] sm:text-xs font-bold"
              style={{
              background: "transparent",
              border: "0",
              color: MONTH_VIEW_HEADING_COLOR,
            ...
            >
              <span className="min-w-0 truncate whitespace-nowrap">
                <span className="month-view-label-text" style={{ color: MONTH_VIEW_HEADING_COLOR }}>{translations.choghadiya || "Choghadiya"}:</span>{" "}
                <span className="month-view-value-text" style={{ color: MONTH_VIEW_CONTENT_COLOR }}>{day.Choghadiya}</span>
              </span>
            </div>
          )}
          ...
          {vShakaSamvat && vShakaSamvat !== "-" && (
            ...
            >
              <span className="min-w-0 truncate whitespace-nowrap">
                <span className="month-view-label-text" style={{ color: MONTH_VIEW_HEADING_COLOR }}>{translations.year || "Year"}:</span>{" "}
                <span className="month-view-value-text" style={{ color: MONTH_VIEW_CONTENT_COLOR }}>{vShakaSamvat}</span>
              </span>
            </div>
          )}
    ...
    return (
      <div className="space-y-3">
        <SectionCard
          ...
          icon="☀"
          variant="sunmoon"
          titleClassName="month-view-white-section-title"
          iconClassName="month-view-white-section-icon"
        >
        ...
        >
          ...
          <div
            ...
            style={{
              background: "transparent",
              border: MONTH_VIEW_SHARED_BORDER,
              boxShadow: "0 0 15px rgba(255, 140, 50, 0.4), inset 0 0 10px rgba(255, 200, 100, 0.1)",
            }}
          >
            <span
              className="month-view-white-section-icon text-base font-black"
              style={{
                color: MONTH_VIEW_SECTION_TITLE_COLOR,
                WebkitTextFillColor: MONTH_VIEW_SECTION_TITLE_COLOR,
                textShadow: MONTH_VIEW_SECTION_TITLE_SHADOW,
              }}
            >
              ✦
            </span>
            <h3
              className="month-view-section-title auspicious-timings-title month-view-white-section-title text-xs sm:text-sm font-black uppercase tracking-wide"
              style={{
                color: MONTH_VIEW_SECTION_TITLE_COLOR,
                WebkitTextFillColor: MONTH_VIEW_SECTION_TITLE_COLOR,
                textShadow: MONTH_VIEW_SECTION_TITLE_SHADOW,
            ...
            >
              {translations.auspiciousTimings || "Auspicious Timings"}
            </h3>
          </div>
          ...
          <InfoRow
            ...
            language={language}
            translations={translations}
            speechKey="Amrit Kalam"
            valueClassName="month-view-time-text"
          />
          )}
          {vAbhijit !== "-" && (
          <InfoRow
                ...
                {[
                  ...
                  return (
                    <button
                      ...
                      className="w-full min-w-0 overflow-hidden rounded-lg px-2 py-2 text-center text-[11px] font-semibold leading-tight whitespace-normal break-words transition sm:text-xs"
                      style={{
                        background: "transparent",
                        border: active
                          ? "2.5px solid #d4a847"
                    ...
                    >
                      <span className="block max-w-full">{translations[dayName] || dayName}</span>
                    </button>
                  );
                })}
              </div>
...
function SectionCard({ title, icon, children, variant, titleClassName = "", iconClassName = "" }) {
  ...
  const consistentBorder = MONTH_VIEW_SHARED_BORDER;
  ...
  return (
    ...
    >
      ...
      >
        <span
          className={`month-view-section-icon text-base font-black ${iconClassName}`.trim()}
          style={{
            color: MONTH_VIEW_SECTION_TITLE_COLOR,
            WebkitTextFillColor: MONTH_VIEW_SECTION_TITLE_COLOR,
            textShadow: MONTH_VIEW_SECTION_TITLE_SHADOW,
        ...
        <h3
          className={`month-view-section-title text-xs sm:text-sm font-black uppercase tracking-wide ${titleClassName}`.trim()}
          style={{
            color: MONTH_VIEW_SECTION_TITLE_COLOR,
            WebkitTextFillColor: MONTH_VIEW_SECTION_TITLE_COLOR,
            textShadow: MONTH_VIEW_SECTION_TITLE_SHADOW,
            ...
            {options.map((optionValue) => {
              ...
              return (
                <button
                  ...
                  style={{
                    background: "transparent",
                    ...headingWhiteTextStyle,
                    borderBottom: "1px solid rgba(255, 255, 255, 0.18)",
                    boxShadow: isSelected ? "inset 0 0 0 1px rgba(90, 150, 69, 0.75)" : "none",
                  }}
                >
                  {optionValue} {suffix}
                </button>
...
function InfoRow({
  ...
  translations,
  speechKey,
  valueColor = MONTH_VIEW_CONTENT_COLOR,
  valueClassName = "",
}) {
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-2">
        <div
          className="month-view-sidebar-label text-xs uppercase tracking-wide mb-1 font-semibold"
          style={{
            color: MONTH_VIEW_HEADING_COLOR,
          }}
        >
        ...
        <SpeakTimingButton
          label={translations?.speak || "Speak"}
          speechKey={speechKey}
          value={value}
          language={language}
        />
      ...
      <div
        className={`month-view-sidebar-value text-sm font-bold ${valueClassName}`.trim()}
        style={{
          color: valueColor,
...
function TimeBox({ label, value, scheme }) {
  ...
  const border = scheme === "orange" || scheme === "blue"
    ? "rgba(255, 183, 77, 0.4)"
    : "transparent";
  ...
  return (
    <div
      ...
      style={{
        background: bg,
        border: border === "transparent" ? "2px solid transparent" : `1.5px solid ${border}`,
        boxShadow:
          "0 0 15px rgba(0,0,0,0.25), inset 0 1px 2px rgba(255,255,255,0.2)",
      }}
    >
      <div
        className="month-view-sidebar-label text-[10px] sm:text-xs font-bold mb-1"
        style={{ color: MONTH_VIEW_HEADING_COLOR }}
      >
        {label}
      </div>
      <div
        className="month-view-time-text text-xs sm:text-sm md:text-base font-black"
        style={{
          color: MONTH_VIEW_TIME_COLOR,
        }}
      >
        {value}
      </div>
...
function AuspiciousBox({ label, value, isAuspicious = true, isToday = false, language, translations }) {
  ...
  return (
    ...
    >
      ...
      <div
        className="month-view-time-text text-sm font-bold"
        style={{
          color: MONTH_VIEW_TIME_COLOR,
        }}
      >
