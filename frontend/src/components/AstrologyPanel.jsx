...
export default function AstrologyPanel({ isOpen, onClose, selectedDay }) {
  ...
  const [kundaliJson, setKundaliJson] = useState(
    JSON.stringify(
      {
        // Fill with local payload fields for your plan.
  ...
  const runRequest = async () => {
    ...
    try {
      if (activeTab === "panchang") {
        const data = await getLocalPanchang({ date, lat, lng });
        setResponse(data);
        return;
      }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 p-4">
          <div className="space-y-3">
            ...
            <div className="text-xs" style={{ color: "rgba(255,228,181,0.75)" }}>
              Note: This calls your backend (`/api/astrology/*`). The calculations stay local to your app.
            </div>
          </div>
