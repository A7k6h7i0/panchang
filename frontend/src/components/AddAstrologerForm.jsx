...
const API_BASE_URL = import.meta.env.VITE_HINDU_SEARCH_API_BASE_URL
  ? import.meta.env.VITE_HINDU_SEARCH_API_BASE_URL
  : "";

export default function AddAstrologerForm({ onAdd, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const postAstrologer = async (body) => {
      ...
      let { response, data } = await postAstrologer(requestBody);
      ...
      if (!response.ok && response.status === 409) {
        ...
        if (looksLikePlaceIdConflict) {
          const retryBody = {
            ...requestBody,
            place_id: normalizedPhone ? `${normalizedPhone}-${Date.now()}` : `astrologer-${Date.now()}`,
          };
          ({ response, data } = await postAstrologer(retryBody));
        }
      }

      if (!response.ok) {
        const message = data?.error || "Failed to add astrologer";
      ...
      setSuccess("Astrologer added successfully!");
      setFormData({
        name: "",
        phone: "",
        address: "",
      ...
      setError(err.message || "Failed to add astrologer");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="relative w-full max-w-md max-h-[90vh] overflow-y-auto bg-white/20 rounded-2xl p-6 mx-auto backdrop-blur-md border border-white/10">
        <div className="flex justify-between items-start mb-4">
          <h2 className="text-xl font-black text-amber-100">
            {t.addAstrologerTitle || "Add Astrologer"}
          </h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              onChange={handleChange}
              className="w-full rounded-xl border border-white/15 bg-black/25 px-3 py-2 text-amber-50 outline-none transition focus:border-amber-300/50 focus:ring-2 focus:ring-amber-300/20"
              placeholder="e.g., horoscope, marriage matching, numerology"
              required
            />
          </div>
          ...
          >
            {loading ? t.adding || "Adding..." : (t.addAstrologerButton || "Add Astrologer")}
          </button>
        </form>
      </div>
    </div>
